...
class SkillTests(unittest.TestCase):
    ...
    def test_builtin_skills_are_available(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(any(item.slug == "species-distribution-and-biodiversity-modeling" for item in skills))
            self.assertTrue(any(item.slug == "ecoacoustic-monitoring-and-bird-observation" for item in skills))
            self.assertTrue(any(item.slug == "movement-ecology-and-telemetry-analysis" for item in skills))
            self.assertTrue(any(item.slug == "open-environmental-data-and-forcing-workflow" for item in skills))
            ...
            self.assertTrue(any(item.slug == "ecology-photogrammetry-and-3d-reconstruction" for item in skills))
            self.assertTrue(any(item.slug == "close-range-ecology-photogrammetry" for item in skills))
            self.assertTrue(any(item.slug == "lidar-point-cloud-and-canopy-analysis" for item in skills))
            self.assertTrue(any(item.slug == "lidar-survey-design-and-simulation" for item in skills))
            self.assertTrue(any(item.slug == "tree-qsm-and-forest-structure-modeling" for item in skills))
            self.assertTrue(any(item.slug == "habitat-scene-3d-visualization" for item in skills))
            self.assertTrue(any(item.slug == "web-geospatial-3d-publishing" for item in skills))
